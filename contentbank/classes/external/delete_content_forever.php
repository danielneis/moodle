...
/**
 * This is the external method for deleting a content forever.
 *
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class delete_content_forever extends external_api {
    /**
     * Parameters.
     *
        ...
        foreach ($params['contentids'] as $contentid) {
            try {
                ...
                if ($contenttype->can_delete($content)) {
                    ...
                    if (!$contenttype->delete_content_forever($content)) {
                        $warnings[] = [
                            'item' => $contentid,
