...
class enrol_database_plugin extends enrol_plugin {
    ...
    /**
     * @var int Remove unused groups.
     */
    const REMOVE_UNUSED_GROUPS = 0;

    /**
     * @var int Keep unused groups.
     */
    const KEEP_UNUSED_GROUPS = 1;
    ...
    public function sync_groups(progress_trace $trace): void {
        ...
        $courseswithgroups = [];
        $sql = $this->db_get_sql($table, [], [$coursefield], true);
...
EOF;
    ...
    protected function sync_groups_for_course(
        ...
        $sql = $this->db_get_sql($table, $params, $fields);
        $rs = $extdb->Execute($sql);
        ...
        if ($removegroups == self::REMOVE_UNUSED_GROUPS) {
...
EOF;
        ...
        foreach ($possiblelocalusers as $useridnumber => $localuser) {
            if (array_key_exists($useridnumber, $remotemembers)) {
                if (empty($localuser->gmid)) {
                    ...
                    group_add_member($localgroup, $localuser->userid, 'enrol_database', 0);
                }
            } else {
                $trace->output("Removing {$localuser->userid} ({$useridnumber}) from {$localgroup->name}");
                group_remove_member($localgroup, $localuser->userid);
            }
        }
    }
    ...
    protected function db_get_sql($table, array $conditions, array $fieldlist, $distinct = false, $sort = "") {
        $fields = [];
