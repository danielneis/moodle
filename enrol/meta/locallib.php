...
function enrol_meta_create_new_group($courseid, $linkedcourseid) {
    ...
    $groupid = groups_create_group($groupdata, false, false, 'enrol_meta', 0);
 ...
 */
function enrol_meta_update_group_itemid($groupid, $instanceid): bool {
    global $DB;
    if (!$groupid) {
        return false;
    }
    if (!$group = $DB->get_record('groups', ['id' => $groupid], 'id')) {
        return false;
    }
    if (!$instanceid) {
        return false;
    }
    if (!$DB->record_exists('enrol', ['id' => $instanceid])) {
        return false;
    }
