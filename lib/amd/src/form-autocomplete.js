...
define([
    ...
    /**
     * Show the list of suggestions using popper
     *
     * @param {Object} state The state variables for this autocomplete.
     */
    var showSuggestions = function(state) {

        // Find the elements in the DOM.
        var inputElement = $(document.getElementById(state.inputId));
        var suggestionsElement = $(document.getElementById(state.suggestionsId));
        var selectionElements = $(document.getElementById(state.selectionId));
        var width = (selectionElements.width() > 400) ? 400 : (selectionElements.width() - 20);

        if (!suggestionsElement.length || !inputElement.length) {
            return;
        }

        // Show it if it is hidden.
        new Popper(inputElement, suggestionsElement, {
