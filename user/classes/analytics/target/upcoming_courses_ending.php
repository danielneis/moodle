...
/**
 * Upcoming courses ending target.
 *
 * @package   core
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class upcoming_courses_ending extends \core_analytics\local\target\binary {
    ...
    public static function get_name() : \lang_string {
        return new \lang_string('target:upcomingcoursesending', 'user');
    }
    ...
    public function get_insight_subject(int $modelid, \context $context) {
        return get_string('youhaveupcomingcoursesending', 'user');
    }
    ...
    public function is_valid_analysable(\core_analytics\analysable $analysable, $fortraining = true) {
        // The calendar API used by \core_course\analytics\indicator\courses_ending is already checking
    ...
    /**
     * Calculation based on courses ending indicator.
     *
     * @param int $sampleid
     * @param \core_analytics\analysable $analysable
     * @param int $starttime
    ...
    protected function calculate_sample($sampleid, \core_analytics\analysable $analysable, $starttime = false, $endtime = false) {
        ...
        $coursesendingindicator = $this->retrieve('\core_course\analytics\indicator\courses_ending', $sampleid);
        if ($coursesendingindicator == \core_course\analytics\indicator\courses_ending::get_max_value()) {
            return 1;
        }
        return 0;
    }
    ...
    public function get_insight_body_for_prediction(\context $context, \stdClass $user, \core_analytics\prediction $prediction,
        ...
        $fullmessageplaintext = get_string('youhaveupcomingcoursesending', 'user', $user->firstname);

        $sampledata = $prediction->get_sample_data();
        $coursesending = $sampledata['core_course\analytics\indicator\courses_ending:extradata'];

        if (empty($coursesending)) {
            // We can throw an exception here because this is a target based on assumptions and we require the
            // activities_due indicator.
            throw new \coding_exception('The courses_ending indicator must be part of the model indicators.');
        }

        $coursestext = [];
        foreach ($coursesending as $key => $courseending) {

            // Human-readable version.
            $coursesending[$key]->formattedtime = userdate($courseending->enddate);

            // We provide the URL to the activity through a script that records the user click.
            $courseurl = new \moodle_url('/course/view.php', ['id' => $courseending->id]);
            ...
            $coursesending[$key]->url = $actionurl->out(false);

            if (count($coursesending) === 1) {
                // We will use this activity as the main URL of this insight.
                $insighturl = $actionurl;
            }

            $coursestext[] = $courseending->shortname . ': ' . $coursesending[$key]->url;
        }
        ...
        $courseshtml = $OUTPUT->render_from_template('core_user/upcoming_courses_ending_insight_body', (object) [
            'coursesending' => array_values($coursesending),
    ...
    public function prediction_actions(\core_analytics\prediction $prediction, $includedetailsaction = false,
        ...
        // We force a lookahead of 30 days so we are sure that the upcoming courses ending are shown.
        $url = new \moodle_url('/calendar/view.php', ['view' => 'upcoming', 'lookahead' => '30']);
        $pix = new \pix_icon('i/course', get_string('viewupcomingcoursesending', 'user'));
        $action = new \core_analytics\prediction_action('viewupcoming', $prediction,
            $url, $pix, get_string('viewupcomingcoursesending', 'user'));
